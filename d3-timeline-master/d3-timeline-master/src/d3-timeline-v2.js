...
(function() {
  d3.timeline = function() {
    ...
    var appendTimeAxis = function(g, xAxis, yPosition) {
      ...
      var axis = g.append("g")
        .attr("class", "axis")
        .attr("transform", "translate(" + 0 + "," + 25 + ")") //moves the X axis
        .call(xAxis);
    };
    ...
    function timeline(gParent) {
      ...
      g.each(function(d, i) {
        ...
        d.forEach(function(datum, index) {
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .attr("yAxisMapping", function(d) {
              return yAxisMapping[index];
            })
            .attr("x", getXPos)
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .attr("textWidth", function(d) {
              return d3.select(this).node().getBBox().width;
            })
            .attr("boxWidth", function(d) {
              return (d.ending_time - d.starting_time) * scaleFactor;
            })
            .attr("id", function(d, i) {
              if (datum.id && !d.id) {
          ...
          g.selectAll("text")
            .text(function(d) {
              ...
              var boxWidth = parseInt(d3.select(this).attr("boxWidth"));
              if (textWidth > boxWidth) {
                var fraction = (boxWidth / textWidth) * d.label.length * 0.7;
                return d.label.substring(0, fraction) + "...";
              } else {
                return d.label;
              }
            });
      ...
      function getXPos(d, i) {
        var barWidth = (d.ending_time - d.starting_time) * scaleFactor;
        var mappedX = (d.starting_time - beginning) * scaleFactor;
        var timeLineWidth = (ending - beginning) * scaleFactor;
        return timeLineWidth - mappedX - barWidth + margin.left;
      }
      ...
      function getXTextPos(d, i) {
        ...
        var mappedX = (d.starting_time - beginning) * scaleFactor;
        var timeLineWidth = (ending - beginning) * scaleFactor;
        return timeLineWidth - mappedX - barWidth + margin.left + 5;
      }
